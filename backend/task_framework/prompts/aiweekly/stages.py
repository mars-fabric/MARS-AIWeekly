...
curation_researcher_prompt = (
    ...
    "\n"
    "URL PRIORITY — when listing Sources for a story, always place official company/org URLs FIRST:\n"
    "  OFFICIAL (put first): the org's own domain (e.g. openai.com, anthropic.com, deezer.com,\n"
    "    doordash.com, nvidia.com, deepmind.google, huggingface.co, arxiv.org, newsroom.*.com,\n"
    "    press.*.com, *.com/blog, *.com/news, *.com/newsroom, prnewswire.com, businesswire.com)\n"
    "  AGGREGATORS (put last): techcrunch.com, theverge.com, venturebeat.com, aibusiness.com,\n"
    "    wired.com, engadget.com, zdnet.com, cnet.com, reuters.com, bloomberg.com, wsj.com\n"
    "  The FIRST URL in Sources is what will appear as the primary link in the final report.\n"
    "\n"
    "Output per item:\n"
    "- **[Title]** | [Organization] | [YYYY-MM-DD]\n"
    "  [Summary from source data only]\n"
    "  Sources: [URL1], [URL2], ...  (official/primary URL first, aggregators last)\n"
    + _OUTPUT_RULES +
    "---\n"
    "Raw Collection:\n"
    "{raw_collection}\n"
...
generation_writer_prompt = (
    ...
    "When a curated item has multiple Sources URLs, use the FIRST one (the most official/primary).\n"
    "Prefer official company/org domains over news aggregators like techcrunch.com or theverge.com.\n"
...
review_editor_prompt = (
    "Edit the draft report below by applying ONLY the critic's corrections — do NOT rewrite from scratch.\n"
    "\n"
    "**Period:** {date_from} to {date_to} | **Style:** {style}\n"
    "\n"
    "PATCH MODE — make the minimum changes needed:\n"
    "- Add missing items from curated data (insert into the correct date section)\n"
    "- Fix fact mismatches: wrong dates, names, orgs\n"
    "- Fix or remove bad/fabricated URLs — use ONLY URLs from curated data\n"
    "- {expand_instruction}\n"
    "- Remove clear duplicates and unsupported superlatives\n"
    "- Ensure all 5 sections exist; add any missing section\n"
    "\n"
    "CRITICAL: Copy every existing highlight from the draft EXACTLY unless the critic flagged it.\n"
    "Do NOT summarize, condense, or drop any item that was NOT flagged by the critic.\n"
    "Every item in Key Highlights MUST end with a clickable [Source](url) link.\n"
    ...
    "{review_critique}\n"
    "\n---\n"
    "Draft Report (copy all content, patch only flagged items):\n"
    "{draft_report}\n"
    "\n---\n"
    "Curated Source Data:\n"
    "{curated_items}\n"
